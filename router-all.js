// router-all.js — Martybot FULL router for Railway
// Telegram + WhatsApp pairing + Web API + original MetaAgent/sub-agents.
// It loads full agents with a safe fallback so one broken agent cannot kill the whole service.
...
async function loadFullAgents() {
  ...
  try {
    ...
    log('✅ Full MetaAgent + sub-agents loaded.');
    return full;
  } catch (e) {
    full = { enabled: false, error: e.message || String(e), meta: null, auto: null };
    errlog('⚠️ Full agents failed to load:', full.error);
    return full;
  }
}
...
async function aiReplyFallback(text) {
  ...
  if (p.provider === 'openrouter') {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + process.env.OPENROUTER_API_KEY, 'HTTP-Referer': process.env.PUBLIC_APP_URL || 'https://railway.app', 'X-Title': 'Martybot' }, body: JSON.stringify({ model: p.model, messages: [{ role: 'system', content: system }, { role: 'user', content: text }] }) });
  ...
  if (p.provider === 'deepseek') {
    const r = await fetch('https://api.deepseek.com/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + process.env.DEEPSEEK_API_KEY }, body: JSON.stringify({ model: p.model, messages: [{ role: 'system', content: system }, { role: 'user', content: text }] }) });
    const data = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(data.error?.message || data.error || 'DeepSeek HTTP ' + r.status);
    return data.choices?.[0]?.message?.content || 'Bez odpovědi.';
  }
  if (p.provider === 'openai') {
    const r = await fetch('https://api.openai.com/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + process.env.OPENAI_API_KEY }, body: JSON.stringify({ model: p.model, messages: [{ role: 'system', content: system }, { role: 'user', content: text }] }) });
    const data = await r.json().catch(() => ({}));
...
function extractWaText(message) {
  return String(message?.conversation || message?.extendedTextMessage?.text || message?.imageMessage?.caption || message?.videoMessage?.caption || message?.buttonsResponseMessage?.selectedButtonId || message?.listResponseMessage?.singleSelectReply?.selectedRowId || '').trim();
}

async function startWhatsApp() {
  if (waStarting) return;
  if (!envFlag('ENABLE_WHATSAPP')) { waMode = 'disabled'; log('WhatsApp disabled. Set ENABLE_WHATSAPP=true.'); return; }
  const phone = phoneNumber();
  if (!phone) { waMode = 'missing-phone'; waLastError = 'Missing WA_PHONE_NUMBER'; log(waLastError); return; }
  waStarting = true;
  try {
    ...
    let logger;
    try { logger = require('pino')({ level: process.env.WA_LOG_LEVEL || 'silent' }); } catch { logger = { child() { return this; }, trace(){}, debug(){}, info(){}, warn(){}, error(){} }; }
    ...
    sock.ev.on('connection.update', (update) => {
      ...
      if (update.connection === 'open') { waConnected = true; waMode = 'connected'; waLastError = ''; log('✅ WhatsApp connected.'); }
      if (update.connection === 'close') { waConnected = false; waMode = 'closed'; waLastError = update.lastDisconnect?.error?.message || 'connection closed'; warn('WhatsApp closed:', waLastError); const code = update.lastDisconnect?.error?.output?.statusCode; if (code !== DisconnectReason?.loggedOut) setTimeout(() => startWhatsApp(), 5000); }
    });
    ...
    waReady = true; waMode = 'socket-ready';
    log('📱 WhatsApp socket ready for ' + phone);
  } catch (e) { waMode = 'startup-error'; waLastError = e.message || String(e); errlog('WhatsApp startup failed:', waLastError); }
...
async function gitStatus() {
  ...
  return ['🧩 Git status', 'Branch: ' + (branch.stdout || 'unknown'), 'Změny: ' + changed, ahead.ok ? 'Sync: ' + ahead.stdout.replace(/\s+/, ' behind / ') + ' ahead' : 'Sync: upstream nenastavený', short.stdout ? '\n' + short.stdout.slice(0, 1200) : 'Working tree čistý ✅'].join('\n');
}

async function gitPull() {
  if (!envFlag('ALLOW_GIT_PULL') && !envFlag('ALLOW_AGENT_WRITE')) return 'Git pull je vypnutý. Nastav ALLOW_GIT_PULL=true.';
...
function formatStatus() {
  ...
  return ['✅ Martybot FULL router běží', 'Mode: ' + s.mode, 'Full agents: ' + (s.fullAgents ? 'ON' : 'OFF'), s.fullAgentsError ? 'Agent error: ' + s.fullAgentsError : '', 'Provider: ' + s.provider, 'Model: ' + s.model, 'AI: ' + (s.aiAvailable ? 'ON' : 'OFF'), 'Telegram: ' + (s.telegramStarted ? 'ON' : 'OFF'), 'WhatsApp: ' + s.whatsappMode, 'Socket ready: ' + s.whatsappSocketReady, 'Connected: ' + s.whatsappConnected, 'Pairing ready: ' + s.whatsappPairingReady, s.whatsappLastError ? 'WA error: ' + s.whatsappLastError : '', s.telegramError ? 'TG error: ' + s.telegramError : ''].filter(Boolean).join('\n');
}

async function handleWithFullAgent(text, platform, userId) {
  if (!full.enabled || !full.meta || typeof full.meta.handle !== 'function') return null;
  const replies = [];
  const reply = async (msg) => chunkText(msg).forEach(p => replies.push(p));
  await full.meta.handle({ userId: userId || platform + '_user', platform, text, reply });
  return replies.join('\n') || 'Hotovo.';
}
...
async function handleCommandOrAI(text, platform = 'web', userId = 'web_user') {
  ...
  if (cmd === '/start' || cmd === '/help') return ['Martybot FULL ✅', '', 'Příkazy:', '/status', '/git', '/git pull', '/wa status', '/wa pair', '/wa reset', '/agent reload', '', 'Normální text jde do původního MetaAgenta/sub-agentů.'].join('\n');
  ...
  try {
    const fullOut = await handleWithFullAgent(raw, platform, userId);
    if (fullOut) return fullOut;
  } catch (e) {
    return 'Chyba původního agenta: ' + (e.message || String(e));
  }
  try { return await aiReplyFallback(raw); } catch (e) { return 'AI chyba: ' + (e.message || String(e)); }
}

async function startTelegram() {
  if (envFlag('DISABLE_TELEGRAM') || !process.env.TELEGRAM_TOKEN) { tgStarted = false; tgError = envFlag('DISABLE_TELEGRAM') ? 'disabled' : 'missing token'; log('Telegram disabled or TELEGRAM_TOKEN missing.'); return; }
  try {
    ...
    tgBot.on('message', async (msg) => {
      const chatId = String(msg.chat.id); const text = String(msg.text || '').trim();
      const allow = allowedTelegramIds();
      if (allow.length && !allow.includes(chatId) && !allow.includes('tg_' + chatId)) { await tgBot.sendMessage(chatId, '🚫 Tenhle Telegram chat není v ALLOWED_TELEGRAM_CHAT_IDS. Tvoje ID: ' + chatId); return; }
      try { const out = await handleCommandOrAI(text, 'telegram', 'tg_' + chatId); for (const part of chunkText(out, 3900)) await tgBot.sendMessage(chatId, part, { disable_web_page_preview: true }); }
      catch (e) { await tgBot.sendMessage(chatId, 'Chyba: ' + (e.message || String(e))); }
    });
    tgBot.on('polling_error', (e) => { tgError = e.message || String(e); errlog('Telegram polling error:', tgError); });
    log('✅ Telegram FULL started.');
  } catch (e) { tgStarted = false; tgError = e.message || String(e); errlog('Telegram startup failed:', tgError); }
}
...
function startHttp() {
  ...
  http.createServer(async (req, res) => {
    ...
    try {
      if (['/', '/health', '/api/status', '/api/whatsapp/status'].includes(url.pathname)) { if (!authOk(req)) return json(res, 401, { ok: false, error: 'Unauthorized' }); return json(res, 200, statusPayload()); }
      if (url.pathname === '/api/whatsapp/pair' && (req.method === 'GET' || req.method === 'POST')) { if (!authOk(req)) return json(res, 401, { ok: false, error: 'Unauthorized' }); return json(res, 200, { ok: true, ...(await requestPairingCode()) }); }
      if (url.pathname === '/api/whatsapp/reset' && (req.method === 'GET' || req.method === 'POST')) { if (!authOk(req)) return json(res, 401, { ok: false, error: 'Unauthorized' }); return json(res, 200, { ok: true, ...(await resetWhatsAppSession()) }); }
      if (url.pathname === '/api/chat' && req.method === 'POST') { if (!authOk(req)) return json(res, 401, { ok: false, error: 'Unauthorized' }); const body = await readJson(req); const text = String(body.text || body.message || '').trim(); const userId = String(body.userId || 'web_user'); const reply = await handleCommandOrAI(text, 'web', userId); return json(res, 200, { ok: true, replies: chunkText(reply) }); }
      return json(res, 404, { ok: false, error: 'Not found', path: url.pathname });
    } catch (e) { return json(res, 500, { ok: false, error: e.message || String(e), status: statusPayload() }); }
  }).listen(port, '0.0.0.0', () => log('🌐 HTTP API listening on 0.0.0.0:' + port + ' router-all'));
}

log('🚀 router-all starting');
startHttp();
loadFullAgents();
startTelegram();
startWhatsApp();

...
async function waReply(userId, text) {
if (!waProvider) return;
const chunks = chunkText(text, 3800);
for (const chunk of chunks) {
await waProvider.sendMessage(`${userId}@s.whatsapp.net`, { body: chunk }, {});
await sleep(300);
}
}
...
const msg = { userId: ctx.from, platform: ‘whatsapp’, text: ctx.body.trim(), reply };
...
// ← KEY CHANGE: usePairingCode + phoneNumber instead of QR scan
// Phone number must include country code, no + or spaces: e.g. 420777123456

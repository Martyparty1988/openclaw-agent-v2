...
function publicStatus() {
  ...
  return {
    ...
    activeApprovals: Number(meta?.pendingApprovals?.size || 0),
    commitRef: process.env.RAILWAY_GIT_COMMIT_SHA || process.env.VERCEL_GIT_COMMIT_SHA || process.env.GIT_COMMIT || 'unknown',
...
function readinessStatus() {
  ...
  const checks = {
    envLoaded: Boolean(process.env.TELEGRAM_TOKEN || process.env.WEB_API_TOKEN),
    providerConfigured: Boolean(keyName ? process.env[keyName] : false),
    memoryConfigured: Boolean(process.env.MEMORY_DIR || process.env.SUPABASE_URL),
    rssMemoryMB: Math.round(process.memoryUsage().rss / 1024 / 1024),
  };
  return { ok: checks.envLoaded && checks.providerConfigured, provider, checks };
}
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/healthz') {
        return sendJson(res, 200, healthStatus());
      }
      ...
      lastApiError = String(err?.message || err);

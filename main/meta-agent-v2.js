...
const COMMANDS = {
  ...
  web: ['web', 'ui', 'frontend', 'ovladani webu', 'ovládání webu'],
...
function parseCommand(text) {
  ...
  for (const cmd of ['git', 'auto', 'agent', 'model', 'proactive', 'web']) {
  ...
  async handle(msg) {
    ...
    try {
      switch (command) {
        ...
        case 'web': {
          const action = String(task || 'status').trim().toLowerCase();
          if (!action || ['status', 'stav', 'help', 'napoveda', 'nápověda'].includes(action)) {
            return reply([
              '🌐 Ovládání webu',
              '• /web status — rychlá nápověda',
              '• /web improve — spustí web-improve audit + automatické úpravy',
              '• /agent web — přepne tě do webového režimu asistenta',
              '',
              'Tip: napiš také /vylepši web nebo /web improve.',
            ].join('\n'));
          }
          if (['improve', 'audit', 'fix', 'update', 'vylepsi', 'vylepši', 'zlepsi', 'zlepši'].includes(action)) {
            await reply('🌐 Web-improve spuštěn přes /web... Analyzuji web, generuji vylepšení a commitnu.');
            setImmediate(async () => {
              try {
          ...
          return reply('❌ Neznámý web příkaz. Použij /web status nebo /web improve.');
        }
...
• /auto run — kompletní auto audit
• /web status — nápověda pro ovládání webu
• /web improve — AI audit + automatické web vylepšení
